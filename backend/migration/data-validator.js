...
class DataValidator {
  ...
  async validateSourceData() {
    ...
    try {
      ...
      results.categories.valid = categories.filter(cat => 
        cat.id && cat.name && typeof cat.displayOrder === 'number'
      ).length;

      // Validate models
      const models = await this.loadLocalData('models.ts');
      results.models.total = models.length;
      results.models.valid = models.filter(model => 
        model.id && model.name && model.categoryId
      ).length;

      this.updateTestStats(3, results.products.valid === results.products.total && 
                             results.categories.valid === results.categories.total && 
                             results.models.valid === results.models.total ? 3 : 0);
  ...
  async validateAssetIntegrity() {
    ...
    try {
      ...
      const allAssetsValid = results.images.accessible === results.images.total && 
  ...
  async validateStrapiSetup() {
    ...
    try {
      ...
      const setupValid = results.connection && 
    ...
    for (const test of endpoints) {
      ...
      try {
        ...
        if (test.expectedMin && response.data && response.data.length >= test.expectedMin) {
          result.status = 'pass';
        } else if (test.expectedFields && response.data && test.expectedFields.every(field => field in response.data)) {
          result.status = 'pass';
  ...
  async validateMediaAccessibility() {
    ...
    try {
      ...
      for (const file of mediaFiles.data.slice(0, 10)) { // Test first 10 files
        const startTime = Date.now();
  ...
  async validateUserWorkflows() {
    ...
    const workflows = [
      ...
      },
      {
        ...
        steps: [
          () => this.makeRequest('/api/products?filters[title][$contains]=tiffosi')
        ]
      }
    ];
    ...
    return { workflows: results, summary: { total: workflows.length, successful: successfulWorkflows } };
  }
  ...
  async generateReport() {
      ...
      if (results.error) {
        ...
        Object.entries(results).forEach(([key, value]) => {
          if (typeof value === 'object' && value.total !== undefined) {
            console.log(`  ${key}: ${value.valid || value.actual}/${value.total || value.expected}`);
          }
        });
      }
    });
    ...
    // Save report to file
    const reportPath = path.join(__dirname, `validation-report-${this.validationResults.phase}-${Date.now()}.json`);
