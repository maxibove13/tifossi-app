...
class MediaAssetUploader {
  ...
  async initialize() {
    ...
    try {
      const response = await axios.get(`${this.strapiUrl}/api/upload/files?pagination[pageSize]=1`, {
  ...
  async validateUploads() {
    ...
    for (const [originalPath, urls] of Object.entries(this.urlMapping.images)) {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          validationResults.images.failed.push({ path: originalPath, error: `HTTP ${response.status}` });
        }
      } catch (error) {
    ...
    for (const [originalPath, urls] of Object.entries(this.urlMapping.videos)) {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          validationResults.videos.failed.push({ path: originalPath, error: `HTTP ${response.status}` });
        }
      } catch (error) {
    ...
    console.log(`\\n📊 Validation Results:`);
    console.log(`Images: ${validationResults.images.accessible}/${validationResults.images.total} accessible`);
    console.log(`Videos: ${validationResults.videos.accessible}/${validationResults.videos.total} accessible`);
    
    if (validationResults.images.failed.length > 0 || validationResults.videos.failed.length > 0) {
      console.log(`\\n❌ Failed validations:`);
      [...validationResults.images.failed, ...validationResults.videos.failed].forEach(failure => {
  ...
  async generateReport() {
    ...
    console.log(`Size reduction: ${(sizeDifference / 1024 / 1024).toFixed(2)} MB (${compressionRatio.toFixed(1)}%)`);
  }
